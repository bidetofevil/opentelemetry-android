...
class ActivityLifecycleInstrumentation : AndroidInstrumentation {
    ...
    override fun install(ctx: InstallationContext) {
        startupTimer.start(ctx.kotlinOtel.tracerProvider.getTracer(INSTRUMENTATION_SCOPE))
    ...
    private fun buildActivityLifecycleTracer(ctx: InstallationContext): DefaultingActivityLifecycleCallbacks {
        ...
        val delegateTracer: Tracer = ctx.kotlinOtel.tracerProvider.getTracer(INSTRUMENTATION_SCOPE)
        val tracers =
            ActivityTracerCache(
